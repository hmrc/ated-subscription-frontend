...
class RegisteredBusinessServiceSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with BeforeAndAfterEach {
  ...
  "RegisteredBusinessService" must {
    ...
    "return review business details, if review details found from Keystore" in {
      ...
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(OK, Json.toJson(testReviewBusinessDetails).toString())))
      val result = testRegisteredBusinessServices.getBusinessCustomerDetails
      await(result) must be(testReviewBusinessDetails)
      verify(mockBusinessCustomerFrontendConnector, times(1)).getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())
    }
    ...
    "return review business details, if review details NOT found from Keystore but there are old mandate details" in {
      ...
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(NOT_FOUND, "")))
      when(mockAgentClientMandateFrontendConnector.getOldMandateDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(Some(oldMandateRef)))
      when(mockAtedConnector.retrieveSubscriptionData(ArgumentMatchers.eq("atedRefNo"))(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(OK, Json.toJson(subscriptionData).toString())))
    ...
    "throw an exception, if review details NOT found from Keystore for old mandate details but no subscription data found " in {
      ...
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(NOT_FOUND, "")))
      when(mockAgentClientMandateFrontendConnector.getOldMandateDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(Some(oldMandateRef)))
      when(mockAtedConnector.retrieveSubscriptionData(ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(INTERNAL_SERVER_ERROR, "")))
    ...
    "throw an exception, if review details from keystore gives back any other status" in {
      ...
      given user: AtedSubscriptionAuthData = AuthBuilder.createAgentAuthContext("userId", "joe bloggs")
      given request: Request[_] = FakeRequest(GET, "")
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(INTERNAL_SERVER_ERROR, "")))
      val thrown = the[RuntimeException] thrownBy await(testRegisteredBusinessServices.getBusinessCustomerDetails)
    ...
    "throw an exception, if review details NOT found and no old mandate details found " in {
      ...
      given request: Request[_] = FakeRequest(GET, "")
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(NOT_FOUND, "")))
      when(mockAgentClientMandateFrontendConnector.getOldMandateDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(None))
      when(mockAtedConnector.retrieveSubscriptionData(ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(INTERNAL_SERVER_ERROR, "")))
      val thrown = the[RuntimeException] thrownBy await(testRegisteredBusinessServices.getBusinessCustomerDetails)
    ...
    "return Correspondence Address if we are an agent, but no address found" in {
      ...
      given user: AtedSubscriptionAuthData = AuthBuilder.createAgentAuthContext("userId", "joe bloggs")
      given request: Request[_] = FakeRequest(GET, "")
      when(mockAtedConnector.getDetails(ArgumentMatchers.any(), ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(NOT_FOUND, "")))
      when(mockBusinessCustomerFrontendConnector.getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(OK, Json.toJson(testReviewBusinessDetails).toString)))
      val result = testRegisteredBusinessServices.getDefaultCorrespondenceAddress()

      await(result).toString must be(testReviewBusinessDetails.businessAddress.toString)
      verify(mockBusinessCustomerFrontendConnector, times(1)).getBusinessCustomerDetails(using ArgumentMatchers.any(), ArgumentMatchers.any())
      verify(mockAtedConnector, times(1)).getDetails(ArgumentMatchers.any(), ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())
    }
    ...
    "return Correspondence Address from the agent if we are one" in {
      ...
      given user: AtedSubscriptionAuthData = AuthBuilder.createAgentAuthContext("userId", "joe bloggs")
      given request: Request[_] = FakeRequest(GET, "")
      when(mockAtedConnector.getDetails(ArgumentMatchers.any(), ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse.apply(OK, successResponse.toString)))
      val result = testRegisteredBusinessServices.getDefaultCorrespondenceAddress()
