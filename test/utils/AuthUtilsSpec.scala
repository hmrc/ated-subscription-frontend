...
class AuthUtilsSpec extends PlaySpec {
  ...
  "isAgent" must {
    "Return true if this user is an agent" in {
      given auth: AtedSubscriptionAuthData = builders.AuthBuilder.createAgentAuthContext("agentId", "Agent Bloggs")
      AuthUtils.isAgent must be(true)
    }

    "Return false if this user is not an agent" in {
      given auth: AtedSubscriptionAuthData = builders.AuthBuilder.createUserAuthContext("userId", "Joe Bloggs")
  ...
  "isAssistant" must {
    "return true if user is an Assistant" in {
      given auth: AtedSubscriptionAuthData = builders.AuthBuilder.createAgentAssistantAuthContext("agentId", "Agent Bloggs")
  ...
  "isAgentUser" must {
    ...
    "return true if user is User Agent" in {
      given auth: AtedSubscriptionAuthData = builders.AuthBuilder.createAgentUserAuthContext("agentId", "Agent Bloggs")
      AuthUtils.isAgentUser must be(true)
    }
  ...
  "agentLink" must {
    "throw Runtime exception is thrown, if user does not have agent account" in {
      given user: AtedSubscriptionAuthData = AuthBuilder.createUserAuthContextWithoutOrg("userId", "joe bloggs")
  ...
  "getArn" must {
    ...
    "throw exception, if agent is not registered and doesn't have ARN in authContext" in {
      given user: AtedSubscriptionAuthData = AuthBuilder.createNotRegisteredAgentAuthContext("userId", "joe bloggs")
