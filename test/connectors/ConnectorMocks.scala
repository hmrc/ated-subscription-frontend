...
trait ConnectorMocks extends FutureAwaits with DefaultAwaitTimeout with MockitoSugar {
  ...
  def execute[A] = requestBuilder.execute[A](any[HttpReads[A]], any[ExecutionContext])
}
