...
class RegisterUserControllerSpec extends PlaySpec with GuiceOneServerPerSuite with BeforeAndAfterEach with AtedTestHelper {
  ...
  val testRegisterUserWithEMACController = new RegisterUserController(mockMCC, mockRegisterUserService, mockAuthConnector,injectedViewInstanceAlreadyRegistered, injectedViewInstanceRegisterUserConfirmation, injectedViewInstanceError)(using mockAppConfig)
          ...
          "return to error page for duplicate users" in {
            registerWithDuplicateUser {
              result =>
                ...
                val document = Jsoup.parse(contentAsString(result))
                document.title() must be("Somebody has already registered from your organisation - Register for ATED - GOV.UK")
            }
          }
      ...
      "confirmation" must {
        "refresh user-profile and view confirmation page" in {
          confirmationWithAuthorisedUser { result =>
            ...
            val document = Jsoup.parse(contentAsString(result))
            document.title() must be("You have successfully registered for ATED - Register for ATED - GOV.UK")
  ...
  def registerWithAuthorisedUser(test: Future[Result] => Any): Unit = {
    ...
    when(mockRegisterUserService.subscribeAted(eqTo(false))(using any(), any(), any(), any()))
      .thenReturn(Future.successful(successResponse))
    when(mockRegisterUserService.enrolAted(eqTo(successResponse), eqTo(false))(using any(), any(), any(), any()))
