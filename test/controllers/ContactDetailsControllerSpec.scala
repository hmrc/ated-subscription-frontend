    ...
    "Authorised Users" must {
      ...
      "return add business details view" in {
        getWithAuthorisedUser(None){ result =>
          ...
          document.select("h1.govuk-fieldset__heading").text() must include("Who should we contact about ATED?")
          document.select("h2.govuk-caption-l >span").text() must be("This section is: ATED registration")
          document.getElementById("contact-details-hint").text() must be("This can be your authorised agent.")
      ...
      "return add business details view for AGENT registering non-uk based clients. Have the correct back link for skip" in {
        getWithAuthorisedAgent(Some("skip")) { result =>
          ...
          document.select("h1.govuk-fieldset__heading").text() must include("Who should we contact about ATED?")
          document.select("h2.govuk-caption-l >span").text() must be("This section is: Add a client")
          document.getElementById("contact-details-hint").text() must be("This could be your contact details as their authorised agent.")
          document.getElementsByAttributeValue("for", "firstName").text() must be("First name")
          document.getElementsByAttributeValue("for", "lastName").text() must be("Last name")
          document.getElementsByAttributeValue("for", "telephone").text() must be("Telephone number")
          document.getElementById("submit").text must be("Continue")
